...
de forma concurrente usando hilos y guarda los resultados incrementalmente.
"""
...
from .utils.helpers import (
    print_progress, current_timestamp, save_progress, load_progress
)
...
class SitiesFetcher:
    """
    Aplicación principal para coordinar el scraping de sitios de forma
    concurrente.
    """

    def __init__(self) -> None:
        """Inicializa el fetcher con su configuración y manejador de datos."""
        self.settings = Settings()
        self.data_handler = DataHandler()
        self._original_sigint_handler = None

    def _setup_signal_handler(self) -> None:
        """Configura el manejador para una interrupción controlada (Ctrl+C)."""
        self._original_sigint_handler = signal.getsignal(signal.SIGINT)
        signal.signal(signal.SIGINT, self._handle_shutdown)

    def _handle_shutdown(self, signum: int, frame: Any) -> None:
        """
        Manejador de la señal de apagado. Activa el evento de parada y
    ...
    def run(
        ...
        try:
            ...
            tasks = df_to_process[['municipio', 'url_municipio']].to_dict(
                'records'
            )
    ...
    def _load_initial_data(self) -> None:
        ...
        print(f"[INFO] Se cargaron {total_sites} sitios existentes.")
    ...
    def _process_results(
        ...
        """Procesa los resultados del scraping y guarda el progreso."""
        for i, result in enumerate(results_iterator):
            if shutdown_event.is_set():
                print("[INFO] Deteniendo el procesamiento de resultados.")
                break

            current_index_in_df = start_index + i
            print_progress(i + 1, total_tasks, "Procesando municipios")
            self._handle_result(result)
            save_progress(current_index_in_df, csv_path, [])

    def _handle_result(self, result: Dict[str, Any]) -> None:
        """Maneja el resultado de una tarea individual de scraping."""
...
def main() -> None:
    """Punto de entrada principal para ejecutar el scraper de sitios."""
    ...
    parser.add_argument(
        '--csv',
        required=True,
        help='Ruta al archivo CSV con las URLs. Ejemplo: '
             '"python -m model_sities.sities_fetcher '
             '--csv data/municipios_urls.csv"'
    )
    parser.add_argument(
        '--start',
        type=int,
        default=0,
        help='Índice de inicio (fila) en el CSV.'
    )
    parser.add_argument(
        '--end',
        type=int,
        default=None,
        help='Índice de fin (fila) en el CSV.'
    )
